...
- **Variants are sparse**: a variant may carry a selection for only *some* of the product's axes — an unset axis simply isn't in `options`, and both UIs render it as `— not set —`. Variants are built one at a time ("Add Variant"); generating the full cross-product is the optional bulk path, not a requirement. Never default an unset axis to the first library value for display: it shows a value that was never stored and doesn't save.
...
- **React admin**: `ProductAttributesPage` (`/admin/products/attributes`), `ProductAttributePresetsPage` (`/admin/products/attribute-presets`), `marketAttributesApi` + `useMarketAttributes`. ProductForm "Attributes" section: add-global-attribute + apply-preset + add-local. Each variant row renders one dropdown per axis; removing an axis or a value **clears the selection** on affected variants rather than deleting them (a variant is hand-built, so its SKU/price shouldn't vanish with an axis). Value lists over 8 collapse behind a `<details>` — a 46- or 89-value store attribute otherwise pushes the variant list off screen.
- **Umbraco plugin**: single Commerce dashboard **"Product Attributes"** tab (list → detail editor: Name/Alias + Values as Name/Alias pairs; the separate "Product Attribute Presets" tab was removed — presets stay in the React admin + API); workspace-view custom-property dropdowns. The product workspace view labels variant axes **"Attributes"** (custom fields are **"Custom properties"**) and supports **global** axes: an "Add store attribute" picker on the Attributes editor, `global`/`local` badges, and per-variant value dropdowns that resolve a global axis's full unique value set from the library (`attributeId` is preserved across all save paths). The "Update Attributes" editor can **remove a global axis from the product** (the library entry is untouched; `_saveOptions` strips it from every existing variant).
