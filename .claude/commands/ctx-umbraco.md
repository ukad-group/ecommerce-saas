...
**Nested / Block List editing**: `products-for-node` and `create` resolve the node's `categoryId` via `GetValueWithAncestorFallback`, so a node nested under folders inherits the category set on an ancestor (e.g. a "Trailer options" root). `product-picker.js` parses the **innermost** `/document/{key}/edit/{key}` segment from the URL so the picker targets the node actually being edited inside a Block List / infinite-editing overlay (not the outer product page). The current selection is shown as a category-independent "Selected: {name}" preview (via `GET product/{id}`) and kept in the list, so the picker never renders empty even when the selected product is outside the resolved category. On a node that hasn't been saved yet `products-for-node` 404s (the node exists only in the browser), so the picker shows an amber "Save and publish this page first" notice plus a **Check again** button instead of an error; an empty list carries the same hint inline.
