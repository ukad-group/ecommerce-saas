...
- Webhook URLs are built from `Payments:PublicBaseUrl`, falling back to the request's own scheme + host when it's blank. The API honours the proxy's `X-Forwarded-Proto` (`app.UseForwardedHeaders`), so that fallback is `https://` behind the TLS-terminating ingress rather than the `http://` Kestrel actually received — a gateway refuses a non-HTTPS callback URL. The Nets provider registers its own subscriptions per payment with a per-payment authorization token
...
- **Tax is sent per line, not as a product** — the Nets Easy request puts `taxRate` (percentage × 100) and `taxAmount` on each goods line, at the rate resolved from the active provider's [Tax Class](TAX-CLASSES.md) for the order's shipping country; `Order.Tax` is allocated across those lines so `order.amount == sum(grossTotalAmount)` still holds. The old `"tax"` / `"payment-fee-tax"` line items are gone (shoppers used to see "Tax" listed as an item on the hosted page)
