...
- Webhook URLs are built from `Payments:PublicBaseUrl`, falling back to the request's host when it's blank — always on **https**, hardcoded rather than read off `Request.Scheme`, because a gateway refuses a non-HTTPS callback URL and TLS terminates at the ingress (Kestrel itself only ever sees plain http). The Nets provider registers its own subscriptions per payment with a per-payment authorization token
